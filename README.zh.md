...
服务端现已提供带 `dak_search` 工具与 OAuth 元数据的 MCP 端点：
...
MCP 工具（所有名称均以 `dak_` 开头）：
- `dak_search`
- `dak_digest_subscribe`
- `dak_digest_editions`
- `dak_digest_edition`
- `dak_api_key_create` —— 为当前登录账号创建 API Key（仅显示一次）
- `dak_digest_publish` —— 生成并发布日报（仅管理员）

旧的 `api_*` 工具名仍会映射到对应的 `dak_*` 工具。
...
curl -sS https://dak-news.com/mcp \
  -H 'content-type: application/json' \
  -H "authorization: ******" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dak_search","arguments":{"q":"关税","category":"finance","limit":5}}}' | jq
```
